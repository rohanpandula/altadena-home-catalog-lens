...
function batchExecute(funcName, callees = [], property = null) {
    for (let i in callees) {
        try {
            ...
            print(`${error} on ${location || "Unknown"} at index ${i} in ${funcName}`);
        }
    }
}
//#endregion
    ...
    for (const a in copyFrom) {
        ...
        if (typeof value === "object" && value !== null) {
            if (value instanceof vec2)
                copyTo[a] = new vec2(value.x, value.y);
            else if (value instanceof vec3)
                copyTo[a] = new vec3(value.x, value.y, value.z);
            else if (value instanceof vec4)
                copyTo[a] = new vec4(value.x, value.y, value.z, value.w);
            else if (value instanceof quat)
                copyTo[a] = new quat(value.x, value.y, value.z, value.w);
            else if (recursive) {
 ...
 * });
 */
function typewriter(text, script, { splitter = "\n", recover = true, skipOn = null, killOn = null, reset = 5, wait = 1, clearAtEnd = true, clearedText = "", timingError = 10, spellErrorChance = 0.01, backspaceTime = 50, callback = null, callbackBySplitter = null, callbackByType = null } = {}) {
    let textBuffer;
    let textInput;
    ...
    update.bind(() => {
        ...
        if (delta >= (reset - noise) * 0.01 && l < lLen) {
            if (Math.random() < spellErrorChance && !misspelled) {
                spellError = String.fromCharCode(97 + Math.floor(Math.random() * 26));
...
class SOGT {
     ...
     */
    play({ cycleCallback = function () { }, callback = function () { }, easeFunction = null, } = {}) {
...
//# sourceMappingURL=HelpersForLS.js.map